...
DataBase::Vector3	DataBase::getInitialDirection(char Type) const {
	return DataBase::Vector3::UNIT_X;
}
DataBase::String		DataBase::getMesh(char Type) const {
	return "robot.mesh";
}
DataBase::String		DataBase::getMaterial(char Type) const {
	switch (Type) {
	case ROBOT:
		return "Robot";
		break;
...
DataBase::Vector3	DataBase::getScale(char Type) const {
	const float DEF = HUMAN_DEFAULT_SCALE;
	switch (Type) {
	case DWARF:
		return DEF*DataBase::Vector3(1.05,0.7,1.1);
		break;
	case GNOME:
		return DEF*DataBase::Vector3(1,0.7,1);
		break;
	case HALFRING:
		return DEF*DataBase::Vector3(0.7,0.7,0.7);
		break;
	case OGRE:
		return DEF*DataBase::Vector3(1.1,1.1,1.15);
		break;
	case ORC:
		return DEF*DataBase::Vector3(1,0.9,1);
		break;
	case GOBLIN:
		return DEF*DataBase::Vector3(1,0.9,1)*0.9;
		break;
	default:
